...
class SunricherService {
    ...
    sendRgbBrightness(brightness, delayAfter = 10) {
        if (brightness !== 0) {
    ...
    getWakeupMessage() {
        return this.getMessage(this.id, 0, 0, 0);
    } 

    getMessage(category, subCategory, value) {
        ...
        this._log.debug(Utils.FormatTrace('getMessage',{category, subCategory, value}, result.map(x => `0x${x.toString(16)}`).join(', ')));
